...
cfg.DATA_LOADER.NUM_WORKERS = 0 # we're setting this to 0 so that it doesn't use multi-processing, but i'm not sure if this is problematic for GPU...
cfg.DATA.PATH_TO_DATA_DIR = "mvit/lesion/01_lesions"
...
cfg.TRAIN.CHECKPOINT_FILE_PATH = "mvit/MViTv2_T_in1k.pyth"
...
def train_epoch(
    ...
    pred_holder = torch.empty(size=[0,5])    
    label_holder = torch.empty(size=[0,5])
    clean_label_holder = torch.empty(size=[0],dtype=torch.int8)
    error_holder=torch.empty(size=[0,5])
...
def train(cfg):
    ...
    start_epoch = 0 # manually set start epoch to 0
    ...
    for cur_epoch in range(start_epoch, cfg.SOLVER.MAX_EPOCH):
        ...
        # lf - saves current weights into folder after every epoch
        with open('current_model_weights.pickle','wb') as mypath:
            pickle.dump(curr_model.state_dict(), mypath)
...
def eval_epoch(val_loader, model, val_meter, cur_epoch, cfg):
    ...
    model.eval()
    val_meter.iter_tic()
    pred_holder = torch.empty(size=[0,5],dtype=torch.float64)    
    label_holder = torch.empty(size=[0],dtype=torch.float64)
    ...
    for cur_iter, (inputs, labels) in enumerate(val_loader):
        ...
        preds = model(inputs)
        # do i need to convert this to float LF
        ...
        # Compute the errors.
        num_topks_correct = metrics.topks_correct(pred_holder, label_holder, (1, 5))
...
# save output weights 
with open('current_model_weights_220924_2.pickle','wb') as mypath:
    pickle.dump(curr_model.state_dict(), mypath)

# load pickle path
with open('current_model_weights.pickle', 'rb') as mypath:
    testweights = pickle.load(mypath)

# # # load model weights
# # now load the model weights again
curr_model.load_state_dict(testweights)
...
# check model inputs make sense 
testloader = build_dataset('Imagenet',cfg,'val')
# test a bunch of predictions and compare with labels
for i in range(26000,26050):
    item = testloader.__getitem__(i)
    img = item[0]
    lab = item[1]
    img = torch.unsqueeze(img,dim=0)
    pred0 = curr_model(img)
    pred1 = torch.argmax(torch.mean(pred0,dim=1),dim=1)
    print(pred1,lab)

# this seems like it pretty mcuh never predicts 0 whcih isn't good
# doesn't predict 1 too well either
# Predicts 3 and 4 pretty well though



# check whether these inputs actually make sense 
import matplotlib.pyplot as plt
testloader2 = build_dataset('Imagenet',cfg,'train')
cur_i = 2
currimage = testloader2.__getitem__(cur_i)[0].permute(1,2,0)
