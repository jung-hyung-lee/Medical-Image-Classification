...
cfg = get_cfg()
cfg['NUM_GPUS'] = 0
cfg.MODEL.NUM_CLASSES = TOTAL_LABEL_COUNT
cfg.MVIT.POOL_KV_STRIDE =[]
...
cfg.DATA.PATH_TO_DATA_DIR = "../DATA/02_eyes_tiny"
...
def train_epoch(
    ...
    pred_holder = torch.empty(size=[0,TOTAL_LABEL_COUNT])
    label_holder = torch.empty(size=[0,TOTAL_LABEL_COUNT])
    clean_label_holder = torch.empty(size=[0],dtype=torch.int8)
    error_holder=torch.empty(size=[0,TOTAL_LABEL_COUNT])
...
if __name__ == '__main__':
    ...
    train(cfg)
    finish = time.time()
    print("***** " + str(round(finish - start, 3)) + " secs *****")



# save output weights 
with open('weights_eye.pickle','wb') as mypath:
    pickle.dump(curr_model.state_dict(), mypath)
